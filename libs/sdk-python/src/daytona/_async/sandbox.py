...
class AsyncSandbox(SandboxDto):
    ...
    async def _experimental_pause(self, timeout: float = 60) -> None:
        ...
        await self._sandbox_api.pause_sandbox(self.id, _request_timeout=timeout if timeout > 0 else None)
        ...
        while self.state == "pausing":
            ...
            if self.state == "error":
                raise DaytonaError(
                    f"Sandbox {self.id} pause failed with state: {self.state}, "
                    f"error reason: {self.error_reason}"
                )
    ...
    async def _experimental_resume(self, timeout: float = 60) -> None:
        ...
        start_time = time.time()
        await self._sandbox_api.resume_sandbox(self.id, _request_timeout=timeout if timeout > 0 else None)
        await self._refresh_data()
        ...
        while self.state == "resuming":
            ...
            if self.state == "error":
                raise DaytonaError(
                    f"Sandbox {self.id} resume failed with state: {self.state}, "
                    f"error reason: {self.error_reason}"
                )
            if 0 < remaining <= time.time() - wait_start:
                raise DaytonaError(
                    f"Sandbox {self.id} failed to resume within {timeout} seconds"
                )
            await asyncio.sleep(check_interval)
            check_interval = min(check_interval * 1.5, 1.0)
